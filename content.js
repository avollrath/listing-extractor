(function () {
  ...
  async function scrapeAllPages(button) {
    ...
    while (currentDoc) {
      ...
      const nextUrl = getNextPageUrl(currentDoc, currentUrl);
      if (!nextUrl) break;

      await new Promise((resolve) => setTimeout(resolve, 400));

      currentUrl = nextUrl;
      currentDoc = await fetchDocument(currentUrl);
      page += 1;
    }
  ...
  function buildOverlay(listings, currentPage, totalPages) {
      ...
      <div id="ts-modal">
        <div id="ts-header">
          <div id="ts-header-left">
            ...
            <span class="ts-badge" id="ts-count">${listings.length} items</span>
            <span class="ts-page-badge" id="ts-page">Scraped ${totalPages} page${totalPages === 1 ? "" : "s"}</span>
          </div>
              ...
              <tbody id="ts-tbody">
                ${listings.map((l, i) => `
                  <tr>
                    ...
                    <td class="ts-name">
                      ${l.link
                        ? `<a href="${esc(l.link)}" target="_blank" rel="noopener noreferrer">${esc(l.name)}</a>`
                        : esc(l.name)}
                    </td>
                    <td class="ts-make">
                      ${l.make
                        ? `<span>${esc(l.make)}</span>`
                        : `<span class="ts-dash">-</span>`}
                    </td>
                    <td class="ts-price">
                      ${esc(l.price) || '<span class="ts-dash">-</span>'}
                    </td>
                  </tr>
                `).join("")}
              </tbody>
        ...
        <div id="ts-footer">
          <span>${listings.length} listings, ${totalPages} page${totalPages === 1 ? "" : "s"}, tori.fi</span>
    ...
    document.getElementById("ts-copy-csv").addEventListener("click", () => {
      ...
      listings.forEach((l, i) => {
        rows.push([i + 1, l.name, l.make, l.price, l.link]);
      });
    ...
    document.getElementById("ts-copy-text").addEventListener("click", () => {
      const text = listings
        .map((l, i) => `${i + 1}. ${l.name}${l.make ? " [" + l.make + "]" : ""}, ${l.price || "n/a"}${l.link ? ", " + l.link : ""}`)
        .join("\n");
